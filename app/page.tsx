...
const generateRandomSquad = () => {
  const t = [...PLAYERS_DB].sort(() => 0.5 - Math.random());
  return {
    titulares: t.slice(0, 11),
    banquillo: t.slice(11, 17),
    extras: t.slice(17, 20)
  };
};

const MOCK_TEAMS_DB = [
  { id: 101, name: "Los Galácticos", user: "CarlosCR7", points: 120, value: 295, squad: generateRandomSquad() },
  { id: 102, name: "La Furia Roja", user: "Ana_Futbol", points: 115, value: 299, squad: generateRandomSquad() },
  { id: 103, name: "Catenaccio FC", user: "Luigi_99", points: 98, value: 280, squad: generateRandomSquad() },
];
       ...
       <div onClick={() => setExpanded(!expanded)} className={`p-4 flex items-center justify-between cursor-pointer hover:bg-white/5 transition-colors`}>
          <div className="flex items-center gap-4">
              ...
              <div>
                  ...
                  <div className="flex items-center gap-1 text-[10px] text-white/50 uppercase font-bold tracking-widest"><User size={10} /> {team.user}</div>
              </div>
          </div>
          <div className="flex items-center gap-4">
         ...
         <div className="border-t border-white/10 bg-[#0d1526] p-4 space-y-4 animate-in slide-in-from-top-2 duration-300">
            <div className="border border-[#22c55e]/20 rounded-2xl bg-[#2e9d4a]/10 p-4 relative overflow-hidden">
              ...
              <div className="flex flex-wrap justify-center gap-2">
                 <div className="w-full flex justify-center gap-2 pb-2 border-b border-white/5">{team.squad.titulares.filter((p:any) => p.posicion === 'DEL').map((p:any) => <MiniPlayer key={p.id} p={p} />)}</div>
                 <div className="w-full flex justify-center gap-2 pb-2 border-b border-white/5">{team.squad.titulares.filter((p:any) => p.posicion === 'MED').map((p:any) => <MiniPlayer key={p.id} p={p} />)}</div>
                 <div className="w-full flex justify-center gap-2 pb-2 border-b border-white/5">{team.squad.titulares.filter((p:any) => p.posicion === 'DEF').map((p:any) => <MiniPlayer key={p.id} p={p} />)}</div>
                 <div className="w-full flex justify-center pt-1">{team.squad.titulares.filter((p:any) => p.posicion === 'POR').map((p:any) => <MiniPlayer key={p.id} p={p} />)}</div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
               <div className="border border-sky-500/20 rounded-2xl bg-sky-900/10 p-3 relative overflow-hidden">
                  ...
                  <div className="grid grid-cols-3 gap-1">{team.squad.banquillo.map((p:any) => <MiniPlayer key={p.id} p={p} small />)}</div>
               </div>
               <div className="border border-white/10 rounded-2xl bg-white/5 p-3 relative overflow-hidden">
                  ...
                  <div className="grid grid-cols-3 gap-1">{team.squad.extras.length > 0 ? team.squad.extras.map((p:any) => <MiniPlayer key={p.id} p={p} small />) : <span className="col-span-3 text-[9px] italic text-white/20 text-center mt-4">N/A</span>}</div>
               </div>
            </div>
         </div>
       )}
...
export default function EuroApp() {
  ...
  const handleFinishSquad = () => {
      setStep(6);
      setIsSaved(true); // AUTO-GUARDADO AL COMPLETAR WIZARD
  }
