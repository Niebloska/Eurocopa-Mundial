...
const PLAYERS_DB = [
  { id: 1, nombre: 'Unai Simón', equipo: 'España', posicion: 'POR', precio: 19, iso: 'es' },
  { id: 2, nombre: 'Neuer', equipo: 'Alemania', posicion: 'POR', precio: 18, iso: 'de' },
  { id: 3, nombre: 'Carvajal', equipo: 'España', posicion: 'DEF', precio: 10, iso: 'es' },
  { id: 4, nombre: 'Saliba', equipo: 'Francia', posicion: 'DEF', precio: 28, iso: 'fr' },
  { id: 5, nombre: 'Rodri', equipo: 'España', posicion: 'MED', precio: 61, iso: 'es' },
  { id: 6, nombre: 'Bellingham', equipo: 'Inglaterra', posicion: 'MED', precio: 64, iso: 'gb' },
  { id: 7, nombre: 'Mbappé', equipo: 'Francia', posicion: 'DEL', precio: 75, iso: 'fr' },
  { id: 8, nombre: 'L. Yamal', equipo: 'España', posicion: 'DEL', precio: 51, iso: 'es' },
];
...
export default function EuroFantasyApp() {
  ...
  const budget = 300;
  const spent = Object.values(selectedPlayers).reduce((acc, p) => acc + p.precio, 0);

  const selectPlayer = (player) => {
    if (activeSlot) {
      if (spent + player.precio > budget) { alert("¡Sin presupuesto!"); return; }
      setSelectedPlayers({ ...selectedPlayers, [activeSlot.id]: player });
      setActiveSlot(null);
    }
  };
    ...
    <div style={{ backgroundColor: '#05080f', minHeight: '100vh', padding: '15px', color: 'white', fontFamily: 'sans-serif' }}>
      <div style={{ maxWidth: '600px', margin: '0 auto' }}>
        ...
        <div style={{ 
          ...
          {/* LÍNEAS TÉCNICAS */}
          <div style={{ 
            position: 'absolute', bottom: '18%', left: '50%', transform: 'translateX(-50%) translateY(50%)', 
            width: '25%', aspectRatio: '1 / 1', border: '2px solid rgba(255,255,255,0.2)', 
            borderRadius: '50%', clipPath: 'inset(0 0 50% 0)', zIndex: 1
          }}></div>
      ...
      {Array.from({ length: count }).map((_, i) => {
        ...
        return (
          <div key={id} onClick={() => onSlotClick({ id, role })} style={{ textAlign: 'center', cursor: 'pointer' }}>
            <div style={{ 
              width: '52px', height: '52px', backgroundColor: player ? '#fff' : 'rgba(0,0,0,0.5)', 
              borderRadius: '14px', border: player ? '2px solid #fff' : '2px solid rgba(255,255,255,0.1)',
